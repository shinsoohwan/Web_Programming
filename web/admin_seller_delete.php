  ...
  for ($i=1; $i<=15; $i++)
	{
      ...
      {
        // 삭제 오류
        echo '<script>alert("삭제 실패.");location.href="admin_seller.php";</script>';
        exit;
      }

			$sql = "DELETE FROM member WHERE id='".$s_id."'";
      $result = $conn->query($sql);
	...
	// 선택한거 모두 삭제 성공
  echo '<script>alert("성공적으로 삭제되었습니다.");location.href="admin_seller.php";</script>';
  exit;
}
else
{
